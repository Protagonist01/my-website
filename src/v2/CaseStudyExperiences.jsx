...
const projectMeta = {
  "retrieval-analytics": {
    ...
    prompt: "How can someone ask a database a question without letting generated SQL run unchecked?",
    decisions: [["Find the schema first", "Retrieve only the tables, columns, and business definitions related to the question."], ["Block unsafe SQL", "Reject writes, unknown fields, suspicious structures, and queries that exceed explicit limits."], ["Show how the answer was made", "Return the SQL, source tables, result, chart, and explanation together."]],
  ...
  "self-healing-monitor": {
    ...
    prompt: "Which recovery actions can run automatically, and which must wait for a person?",
    decisions: [["Gather evidence before acting", "Combine the alert with logs, deployment context, service health, and the matching runbook."], ["Use rules to control permission", "The allowlist, evidence, risk, and blast radius decide whether an action may run."], ["Keep risky actions human-owned", "An operator approves higher-risk actions, and the system records every decision and attempt."]],
  ...
  "ai-voice-receptionist": {
    ...
    prompt: "How can a natural phone call produce exact booking details and a clear next step?",
    decisions: [["Track the request and missing details", "The system records the caller's goal and the information still needed at every step."], ["Use tools for operational facts", "Availability, booking, and messaging accept validated fields instead of conversational guesses."], ["Read everything back", "The caller confirms the details before booking, or reaches a person with the collected context."]],
  ...
  "code-review-agent": {
    ...
    prompt: "How can an AI review changed code without reading the whole repository or posting vague comments?",
    decisions: [["Review changed hunks, not the whole repository", "Only the changed lines and nearby relevant context enter the review."], ["Require a structured finding", "Every comment needs a file, line, severity, explanation, and suggested fix."], ["Make the job inspectable", "The workflow exposes job state, prompts, metrics, validation failures, and the final output."]],
  ...
  framewise: {
    ...
    prompt: "How can a team keep one approved visual direction consistent across every campaign format?",
    decisions: [
      ["Save the approved visual rules", "Store the palette, lighting, composition, typography, and references—not only the prompt."],
      ["Keep work in one versioned canvas", "Generation, editing, and format adaptation remain part of the same campaign history."],
      ["Show what must not change", "The team can lock approved visual decisions before generating another format."],
    ],
  ...
  threadmark: {
    proof: "Interactive product concept",
    prompt: "How can a short research brief remain easy to verify?",
    decisions: [
      ["Attach a source to every claim", "Each generated statement links to the exact passage or media segment that supports it."],
      ["Show conflicting evidence", "Contradictory sources appear as a review task instead of being blended into one confident answer."],
      ["Use one evidence model", "Documents, web pages, recordings, and images enter the same searchable research trail."],
    ],
  ...
  cartpilot: {
    ...
    prompt: "How can a shopping assistant recommend the right product without taking control away from the customer?",
    decisions: [
      ["Translate needs into filters", "The shopper describes the outcome, and the assistant maps it to catalogue constraints."],
      ["Compare before recommending", "Price, features, policies, and trade-offs remain visible beside the recommendation."],
      ["Ask before changing the cart", "The proposed cart change remains pending until the customer approves it."],
    ],
  ...
  marginguard: {
    ...
    prompt: "Where is profit being lost after the costs behind each order are included?",
    decisions: [
      ["Calculate profit per order", "Reconcile revenue with discounts, returns, product cost, shipping, and fulfilment."],
      ["Rank losses the team can fix", "Prioritize repeated leaks by the contribution margin that could realistically be recovered."],
      ["Preview a rule before applying it", "Show the modeled effect of a discount or fulfilment guardrail before operations change."],
    ],
  ...
  "clear-skin": {
    proof: "Built product / Public repository",
    prompt: "How can an AI concierge guide a customer without silently changing a cart or booking?",
    decisions: [
      ["Handle known rules before generation", "Safety checks and predictable requests are routed before the AI agent is used."],
      ["Return a typed proposal", "The interface receives a product, quiz, cart, or booking proposal it can validate and display."],
      ["Ask before important actions", "The customer sees and confirms cart or booking changes before they run."],
    ],
  ...
  "testimony-operations": {
    ...
    prompt: "How can one operator manage intake, editing, approval, and publishing without losing human accountability?",
    decisions: [
      ["Put every stage in one queue", "Collection, editing, approval, and publishing share one visible status history."],
      ["Let AI prepare, not approve", "Summaries and suggested edits reduce repeated reading, but a person makes the final decision."],
      ["Show exceptions clearly", "The operator can see what is blocked, changed, approved, or published without chasing messages."],
    ],
  ...
  "fruit-quality": {
    ...
    prompt: "How can a model result become a quality decision that a user can inspect?",
    decisions: [
      ["Check the image first", "The product verifies that the uploaded image is suitable before running inference."],
      ["Show more than a ripeness label", "The result combines ripeness probabilities with six post-harvest quality forecasts."],
      ["Keep uncertainty visible", "Low-confidence or unsuitable samples are marked for a new image or human inspection."],
    ],
