...
export function useCinematicScroll(containerRef) {
  ...
  useLayoutEffect(() => {
    ...
    const applySectionStates = () => {
      ...
      const stepX = cardBasis ? ((cardBasis.width + diagonalGapPx) / window.innerWidth) * 100 : 44
      const stepY = cardBasis ? ((cardBasis.height + diagonalGapPx) / window.innerHeight) * 100 : 34
      const currentEntryX = lerp(8, 0, entryReveal)
      const currentEntryY = lerp(116, 0, entryReveal)
      ...
      autoCards.forEach((card, index) => {
        ...
        const x = 42 + relative * stepX + currentEntryX
        const y = 38 + relative * stepY + currentEntryY - exitLiftVh
